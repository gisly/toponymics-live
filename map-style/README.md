...
- `style.json` — стартовый минимальный стиль (только background). После загрузки `russia-asian.pmtiles` его надо расширить полным набором слоёв.
...
4. Положить в `pmtiles-local/russia-asian.pmtiles` (для dev) или на VPS в `/opt/toponymics/pmtiles/`
...
2. `java -jar planetiler.jar --download --area=siberia --output=russia-asian.pmtiles`
...
4. Задать data source как `pmtiles://russia-asian.pmtiles` (потребуется локальный сервер с CORS)
